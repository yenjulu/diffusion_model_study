...
def get_pc_radon_ADMM_TV_mri(device, model_conf, ckt_path, kspace, sens, phase_echo, phase_slice, sde, predictor, corrector, inverse_scaler, snr, mask=None,
                             n_steps=1, probability_flow=False, continuous=False,
    ...
    under_kspace = kspace * mask  # mask * kspace
    Sense = mri.Sense(sens, under_kspace,
    ...
    T = Trafos(ishape, contrasts_in_channels=True)
    print('> Trafos oshape: ', T.oshape) # (s, 2 * d, h, w)

    NN_Module = resnet.ResNet2D(in_channels=T.oshape[1],
    ...
    best_checkpoint = torch.load(ckt_path)
    # print("Parameter names in 'model':", best_checkpoint['model_state'].keys())

    renamed_ckt = best_checkpoint["model_state"]
    renamed_ckt = {k.removeprefix("NN_Module.") if k.startswith("NN_Module.") else k: v for k, v in renamed_ckt.items()}

    # NN_Module.load_state_dict(renamed_ckt, strict=False)
    # rho = renamed_ckt.get("ADMM_rho")
    # lamb_1 = renamed_ckt.get("lamda")
    print("rho & lamda:", rho, lamb_1)

        
    def _A(x):
        # print("x1:", x.shape) # torch.Size([s, 2 * d, h, w])
        s, d2, h, w = x.shape
        d = d2 // 2
        x = x.contiguous()
        x = torch.view_as_complex(x.reshape(shape=(s, d, 2, h, w)).permute(0, 1, 3, 4, 2).contiguous()) # (s, d, h, w)
        x = x.view(1, d, 1, 1, s, h, w)  
        return Sense(x)
        # return fft2(x) * mask

    def _AT(kspace):
        img = Sense.adjoint(kspace) # torch.Size([1, d, 1, 1, s, h, w])
        img = img.squeeze() # (d, s, h, w)
        d, s, h, w = img.shape
        real_part, imag_part = torch.real(img), torch.imag(img)
        img = torch.stack((real_part, imag_part), dim=-1) # (d, s, h, w, 2)
        img = img.view(s, 2 * d, h, w)  # # torch.Size([s, 2 * d, h, w])
        return img
    ...
    def CS_routine(x_init, ATy, niter=20):  
        x = x_init.clone()
        v = x_init.clone()
        u = x_init.clone()
        ...
        for i in range(niter):           
            AHA = lambda x: Sense.adjoint(Sense(x)) + rho * x  
            # AHy = ATy + rho * (v - u)
            AHy = ATy + rho * (_DzT(v) - _DzT(u))
            # AHy = ATy + rho * ((_DxT(del_x) - _DxT(udel_x))
            #                     + (_DyT(del_y) - _DyT(udel_y))
            #                     + (_DzT(del_z) - _DzT(udel_z)))
            
            # print('shape of AHy:', AHy.shape)  # torch.Size([1, 14, 1, 1, 3, 200, 200])
            # update x
            x = conj_grad(AHA, AHy, x, max_iter=6)
            ...
            v = T(v)
            v = v.float()
            v = T.adjoint(shrink(v, lamb_1 / rho))
            
            # update u
            # u = u + x - v
            u = u + _Dz(x) - v
            
            # del_x = T.adjoint(shrink(T(_Dx(x) + udel_x).float(), lamb_1 / rho))
            # del_y = T.adjoint(shrink(T(_Dy(x) + udel_y).float(), lamb_1 / rho))
            # del_z = T.adjoint(shrink(T(_Dz(x) + udel_z).float(), lamb_1 / rho))
            # udel_x = _Dx(x) - del_x + udel_x
            # udel_y = _Dy(x) - del_y + udel_y
            # udel_z = _Dz(x) - del_z + udel_z
    ...
    def get_ADMM_TV_fn():
        def ADMM_TV_fn(x, measurement=None):
            with torch.no_grad():
                ATy = Sense.adjoint(Sense.y) # torch.Size([1, d, 1, 1, s, h, w])
                assert x.shape == ATy.shape
                x, x_mean = CS_routine(x, ATy, niter=model_conf['N_unroll'])
                return x, x_mean
        return ADMM_TV_fn
    ...
    def pc_radon(model, data, measurement=None):
        with torch.no_grad():
            x = sde.prior_sampling(data.shape).to(data.device) # torch.Size([s, 2 * d, h, w])   
            # x = data         
            timesteps = torch.linspace(sde.T, eps, sde.N)
            ...
            for i in tqdm(range(sde.N)):                   
                ...
                x_agg = list()
                for idx, x_batch_sing in enumerate(x_batch):
                    # print("shape & dtype:", x_batch_sing.shape, data.shape, x_batch_sing.dtype, data.dtype)
                    # torch.Size([3, 6, 200, 200]) torch.Size([3, 6, 200, 200]) torch.float32 torch.float32
                    x_batch_sing, _ = predictor_denoise_update_fn(model, data, x_batch_sing, t)
                    x_batch_sing, _ = corrector_denoise_update_fn(model, data, x_batch_sing, t)
                    x_agg.append(x_batch_sing)
                ...
                x = torch.cat(x_agg, dim=0)
                x = T.adjoint(x)
                # print(x.shape) # torch.Size([3, 6, 200, 200]) --> torch.Size([1, 3, 1, 1, 3, 200, 200])
                
                # 4. Run ADMM TV
                # print('measurement:', measurement.shape)
                
                # x = T.adjoint(data)
                x, x_mean = mc_update_fn(x, measurement=measurement) # x needs [1, d, 1, 1, s, h, w]
                x, x_mean = T(x).to(torch.float32), T(x_mean).to(torch.float32)
            ...
            x, x_mean = T.adjoint(x), T.adjoint(x_mean)
            return inverse_scaler(x_mean if denoise else x)
