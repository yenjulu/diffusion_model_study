...
class fastmri_knee(Dataset):
    def __init__(self, root, is_complex=False):
        ...
        self.data = self.h5_file['ZS'] # (d, s, h, w)

    def __len__(self):
        return self.data.shape[0]

    def __getitem__(self, idx):
        data = self.data[idx] # (s, h, w)
    ...
    def __del__(self):
        # Make sure to close the HDF5 file when the object is deleted
        self.h5_file.close()
...
def create_dataloader(configs, evaluation=False, sort=True):
  ...
  if configs.data.dataset == 'Object5':
    ...
    if configs.data.magpha:
      ...
      train_dataset = fastmri_knee(Path(configs.data.root), is_complex=True)
      val_dataset = fastmri_knee_infer(Path(configs.data.root), is_complex=True)
