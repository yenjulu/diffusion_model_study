...
def get_pc_radon_ADMM_TV_mri(Sense, model, device, sde, predictor, corrector, inverse_scaler, 
    ...
    def padding(x):
        ...
        # Pad in (left, right, top, bottom) = (w_left, w_right, h_top, h_bottom)
        pad = (pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2)
        x_padded = F.pad(x, pad, mode='constant', value=0)
        return x_padded

    def center_crop(tensor, target_height=384, target_width=384):
    ...
    def CS_routine(x_init, t, niter=20):  
        ...
        for i in range(niter):                                             
            ...
            AHA = lambda x: center_crop(Sense.adj(Sense.fwd(padding(x)[0])).unsqueeze(0)) + rho * x
            AHy = ATy + (rho) * (v - u)
